...
st.title("🔬 Registro y Analítica de Resistencia")

# --- 3. FORMULARIO DE REGISTRO ---
with st.expander("📝 Nuevo Registro de Muestra", expanded=True):
    with st.form("formulario_lims"):
        ...
        for i, atb in enumerate(atbs_keys):
            with cols[i % 6]:
                resultados[atb] = st.selectbox(atb.upper(), ["S", "I", "R"], key=f"form_{atb}")

        if st.form_submit_button("💾 REGISTRAR MUESTRA"):
            if not codigo_id:
                st.warning("⚠️ Debes ingresar un Código de Muestra.")
            else:
                datos = {
                    "codigo_muestra": codigo_id,
                    "origen": origen, 
                    "año": anio, 
                    "provincia": provincia,
                    "programa": programa, 
...
# --- 4. ANALÍTICA CON MEMORIA (SESSION STATE) ---
st.markdown("---")
st.header("📊 Analítica de Prevalencia")

# Botón para cargar datos
if st.button("🔄 Cargar/Actualizar Datos de la Nube"):
    try:
        res = supabase.table("registros_resistencia").select("*").execute()
        if res.data:
            st.session_state['df_lims'] = pd.DataFrame(res.data)
...
# Si hay datos cargados, mostramos los gráficos
if 'df_lims' in st.session_state:
    df = st.session_state['df_lims']
    ...
    with col_chart:
        st.subheader("Resistencia Global por Antibiótico")
        fig = px.bar(df_prev, x="Antibiótico", y="Prevalencia (%)",
                     text_auto='.1f', color="Prevalencia (%)",
    ...
    # --- Gráfico 2: Filtro Dinámico (No desaparece al cambiar) ---
    st.markdown("---")
    st.subheader("🔍 Perfil Específico por Bacteria")
    
    lista_bacterias = df["bacteria"].unique()
    bact_filtro = st.selectbox("Selecciona una bacteria:", options=lista_bacterias)
    ...
    fig_bact = px.line(pd.DataFrame(prev_bact), x="ATB", y="R %", 
                       markers=True, title=f"Perfil de Resistencia: {bact_filtro}")
    fig_bact.update_yaxes(range=[-5, 105])
